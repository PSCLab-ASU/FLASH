...
extern size_t get_workitem_idx();
                        ...
                        float * energy, size_t ** ttable)
{
  ...
  //std::cout << "**" <<std::endl;
  //std::cout <<  ttable[work_item_id][1] << std::endl;
  //std::cout << " ----" <<std::endl;
  size_t adj_stage_idx = ttable[work_item_id][1];
  //std::cout << " WI = " << work_item_id << ", " << adj_stage_idx << std::endl;
  ...
  else
  {
    if( particle_idx == 0 )
    {
      std::cout << " Calling reduction...." << std::endl;
      float energy_;
